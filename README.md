...
```bash
ttrends-api$ sam local invoke GetTrendsFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
ttrends-api$ sam local start-api
ttrends-api$ curl http://localhost:3000/
```
...
```bash
ttrends-api$ sam logs -n GetTrendsFunction --stack-name ttrends-api --tail
```
